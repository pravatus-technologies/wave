...
export const TranslationProvider = ({
  ...
  const getLocale = useCallback(async () => {
    const supportedLocales: string[] = SUPPORTED_LOCALES.split(",").map((l) =>
      l.toLowerCase()
    );
    const fallback = "en";
    ...
    try {
      ...
      if (
        storedLocale &&
        supportedLocales.includes(storedLocale.toLowerCase())
      ) {
      ...
      const deviceLocale =
        Localization.getLocales()?.[0]?.languageTag ?? fallback;
